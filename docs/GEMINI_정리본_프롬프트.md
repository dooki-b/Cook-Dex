...
- **🚨 3D 애니메이션 및 그림자 레이아웃 완전 안정화 (최종)**:
  - **Babel 인프라 수립**: `babel.config.js` 생성 및 `react-native-reanimated/plugin` 주입 (캐시 클리어 필수).
  - **Float 토글 런타임 픽스**: `withSpring` 정밀 소수점 오차로 튕기던 `=== 0` 토글 논리 구조를 State 주축 안전 레이어로 전원 보수했습니다.
  - **통합 섀도우 래퍼(`dashboardWrapper`)**: 각 패널의 그림자를 전면 제거하고 외곽 단일 노드에서 일괄 섀도우를 드리워 이음매 얼룩을 지웠습니다.

**7-19. 게이미피케이션 2.0 (1, 2단계) — 업적 보드, 키친 도감, 별사탕 상점**
- **퀘스트(`quest.tsx`) - 신규 EXP 계산 & 업적 보드**:
  - `100 * (level ^ 1.8)` 새 레벨 공식 도입 (최대 레벨 999 확장).
  - **업적 보드**: 3가지 도전과제(미라클 모닝 셰프, 계란 애호가 등). 해금 시 컬러 + `Shadows.glow`, 잠금 시 grayscale + 불투명도 0.4 톤 분기. 상세 조회 바텀 시트 연동.
- **도감(`profile.tsx`) - 키친 archive**:
  - `react-native-svg` `<Circle>`을 이용한 **도낫 차트** 적용, 5개 요리 카테고리(한식,중식,양식,일식,베이킹) 진척도를 3열 카드 그리드에 대형화 룩으로 시각화.
- **상점(`profile.tsx`) - 별사탕 경제 & 셰프 카드 상점**:
  - 기존 코인 경제를 **별사탕(`starCandy`)**으로 이전. `SKIN_CATALOG` 3종(퓨어 화이트, 미드나이트 블랙, 골든아워) 가로 스크롤 상점 바텀 시트 구현.
  - **`getContrastYIQ` 대비 로직**: 장착 스킨 배경 밝기를 YIQ 공식으로 연산해 검정/흰색 자동 폰트 배색 스위칭.
  - 상점 디자인 팝업을 전면 바텀 시트 미리보기 카드(Live Preview) 형태로 완전 대체.
