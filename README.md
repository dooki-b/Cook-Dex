...
- **기능별 구현 정리**  
  ...
  - create-recipe: Gemini 연동, 3D Cover Flow, DEV 목업, 완성 레시피에서 **기본 계량 가이드** 버튼 노출, **구조화된 스타일 입력 모달(원하는 요리/조리/스타일 추가)**, 광장 공유 전 `"셰프의 킥!"` 사진·텍스트 입력 모달, 광장용 ID 분리(릴레이 메타데이터 포함) 등.
