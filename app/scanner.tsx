...
const callGeminiAPI = async (systemPrompt, imageParts = []) => {
  ...
  for (const model of modelsToTry) {
    try {
      ...
      if (!response.ok) {
        lastError = new Error(
          `[${response.status}] ${model}: ${data.error?.message || "통신 에러"}`,
        );
...
export default function ScannerScreen() {
  ...
  const [selectedPhotoForTraining, setSelectedPhotoForTraining] =
    useState(null);
  ...
  useEffect(() => {
    if (params.manualInput) {
      const inputStr = Array.isArray(params.manualInput)
        ? params.manualInput[0]
        : params.manualInput;
      if (inputStr) {
        const newIngredients = inputStr
          .split(",")
          .map((s) => s.trim())
          .filter((s) => s.length > 0);
          ...
          if (limitDataRaw) {
            ...
            if (limitData.date === today) {
              ...
              await AsyncStorage.setItem(
                "cookdex_daily_scans",
                JSON.stringify({ date: today, count: DAILY_SCAN_LIMIT }),
              );
            }
          } else {
            await AsyncStorage.setItem(
  ...
  const playMockAd = () => {
    ...
    const timer = setInterval(async () => {
      ...
      if (timeLeft <= 0) {
        ...
        Alert.alert(
          "보상 획득! 🎁",
          "스캔 횟수가 1회 충전되었습니다! 바로 스캔을 진행해주세요.",
        );
      }
    }, 1000);
  };
  ...
  const toggleManualIngredient = (ing) => {
    if (manualIngredients.includes(ing))
  ...
  const addCustomManualIngredient = () => {
    ...
    if (newIng && !manualIngredients.includes(newIng))
      setManualIngredients((prev) => [...new Set([...prev, newIng])]);
    setManualInputText("");
  };

  const filteredManualIngredients = COMMON_INGREDIENTS.filter(
    (i) => i.includes(manualInputText) && !manualIngredients.includes(i),
  );
  ...
  const handleRecipeGeneration = () => {
    ...
    if (photos.some((p) => !p.label || p.label.trim() === "")) {
      Alert.alert(
        "식재료로 인식되지 못함",
        "이름이 없는 사진이 있습니다. 사진을 눌러 정확한 식재료 이름을 기입해주세요.",
      );
      return;
    }
  ...
  const generateFromTextOnly = async (customStyleStr = "") => {
    ...
    try {
      ...
      const allIngredients = [
        ...new Set([...currentIngredients, ...manualIngredients]),
      ];
  ...
  const generateFromImage = async (customStyleStr = "") => {
    ...
    try {
      ...
      if (
        ...
        Alert.alert(
          "🚨 구글 API 거절됨",
          `현재 사용 중인 API 키에 권한이 없거나 한도를 초과했습니다.\n\n원인: ${error.message}`,
        );
        setAppStep("camera");
        setIsCurating(false);
        setIsAnalyzing(false);
  ...
  const generateFinalRecipe = async (theme) => {
    ...
    try {
      ...
      { 
        ...
        "nutrition_info": { "calories": 0, "carbs": 0, "protein": 0, "fat": 0 },
        "recipe_markdown": "무조건 첫 줄은 '# ${theme.theme_title}'. 영양 정보는 텍스트로 적지 마. 조리 순서는 '1. ', '2. ' 숫자로 시작할 것." 
      }`;
      ...
      if (parsedData.shopping_list && Array.isArray(parsedData.shopping_list))
      ...
      Alert.alert("🚨 레시피 생성 실패", `상세 원인: ${error.message}`);
      setAppStep("camera");
    } finally {
  ...
  const submitTrainingData = async () => {
    ...
    if (!manualIngredients.includes(trainingInput.trim())) {
      setManualIngredients((prev) => [
        ...new Set([...prev, trainingInput.trim()]),
      ]);
    }
          ...
          onPress: async () => {
            try {
              ...
              if (currentUser) {
                ...
                Alert.alert(
                  "제출 완료! 🎉",
                  "AI 학습 데이터로 소중하게 쓰입니다. 심사 통과 이후 EXP가 지급됩니다!",
                );
                setShowTrainingModal(false);
              }
            } catch (error) {
              Alert.alert("에러", "제출에 실패했습니다.");
            }
          },
        },
      ],
    ...
    try {
      ...
      if (currentUser) {
        ...
        if (isSharing) {
          ...
          Alert.alert(
            "광장에 등록 완료! 🌍✨",
            `스캔한 레시피를 공유하여 엄청난 보상을 받았습니다! (+${earnedExp} EXP)`,
          );
        } else
          Alert.alert(
            "내 주방 저장 완료! 🍳",
            `스캔 레시피가 비밀스럽게 저장되었습니다. (+${earnedExp} EXP)`,
          );
      }
  ...
  const startCookingMode = () => {
    if (!recipeResult) return;
    const extractedSteps = recipeResult
      .split("\n")
    ...
    return (
      <View style={styles.resultBg}>
        <View style={styles.resultContainer}>
          {isAnalyzing && isCurating && (
            <View style={styles.loadingBox}>
              <ActivityIndicator size="large" color="#FF8C00" />
              <Text style={styles.loadingText}>냉장고를 분석 중입니다...</Text>
            </View>
          )}
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {curationThemes.map((theme, index) => (
                  ...
                  >
                    ...
                    <Text
                      style={{ fontSize: 13, color: "#8C7A76", marginTop: 5 }}
                    >
                      {theme.match_reason}
                    </Text>
                    <View
                ...
                <View style={styles.nutritionCard}>
                  ...
                  <View style={styles.disclaimerBox}>
                    <Text style={styles.disclaimerText}>
                      ⚠️ AI가 식약처 DB를 참고하여 추정한 값이며, 실제 조리법과
                      식재료 상태에 따라 오차가 발생할 수 있습니다.
                    </Text>
                  </View>
                </View>
              )}
        ...
        <View style={styles.previewControlsGrid}>
          {!isGeneratingRecipe && recipeResult && (
            <>
              ...
              >
                <Text style={styles.buttonText}>광장 공유 🌍</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
                  styles.gridButton,
              ...
              >
                <Text style={styles.buttonText}>레시피 수정 🎲</Text>
              </TouchableOpacity>
            </>
          )}
          {!isAnalyzing && !isCurating && curationThemes && !recipeResult && (
            <TouchableOpacity
              style={[styles.gridButton, { backgroundColor: "#444", flex: 1 }]}
              onPress={handleExitGuard}
            >
        ...
        <View style={{ paddingBottom: 20, alignItems: "center" }}>
          {!isAnalyzing && !isGeneratingRecipe && recipeResult && (
            <TouchableOpacity
              style={[
                styles.gridButton,
                { backgroundColor: "#444", width: "90%" },
              ]}
              onPress={handleExitGuard}
            >
          ...
          >
            <View
              style={[
                ...
                { height: "auto", maxHeight: "80%", paddingBottom: 30 },
              ]}
            >
              ...
              <ScrollView>
                ...
                <View style={styles.styleModalButtons}>
                  ...
                  >
                    <Text style={styles.styleModalBtnTextWhite}>
                      추천받기 ✨
                    </Text>
                  </TouchableOpacity>
                </View>
    ...
    <View style={styles.container}>
          ...
          {manualIngredients.length > 0 && (
            <View style={styles.addedManualIngredientsBox}>
              <Text style={{ color: "#fff", fontSize: 13, fontWeight: "bold" }}>
                추가됨: {manualIngredients.join(", ")}
              </Text>
            </View>
          )}
          {photos.length > 0 && (
            <View>
              <Text style={styles.trainingHintText}>
                💡 사진을 터치해 AI를 학습시켜주세요!
              </Text>
              <ScrollView
                horizontal
            ...
            <View style={styles.controlsArea}>
              {(photos.length > 0 || manualIngredients.length > 0) && (
                <View style={styles.scannerActionRow}>
                  <TouchableOpacity
                    style={[
                      styles.analyzeMultiButton,
                      { backgroundColor: "#5A4E49" },
                    ]}
      ...
      >
        ...
        >
          <View
            style={[
              styles.bottomSheetContainer,
              { height: "auto", maxHeight: "85%", paddingBottom: 30 },
            ]}
          >
            <View style={styles.dragHandle} />
            ...
            >
              ...
              <View style={styles.quickTagsContainer}>
                {COMMON_INGREDIENTS.filter(
                  (i) => !manualIngredients.includes(i),
                )
                  .slice(0, 7)
                  .map((ing) => (
                    <TouchableOpacity
                      ...
                      onPress={() => toggleManualIngredient(ing)}
                    >
              ...
              <View style={styles.inlineInputRow}>
                ...
                <TouchableOpacity
                  ...
                  onPress={addCustomManualIngredient}
                >
              ...
              {manualInputText.length > 0 && (
                <View style={styles.autocompleteContainer}>
                  {filteredManualIngredients.length > 0 ? (
                    ...
                    <TouchableOpacity
                      style={styles.customAddBadge}
                      onPress={addCustomManualIngredient}
                    >
                      <Text style={styles.customAddText}>
                        '{manualInputText}' 직접 추가 ➕
                      </Text>
                    </TouchableOpacity>
                  )}
                </View>
              ...
              >
                ...
                >
                  ...
                  <TouchableOpacity onPress={() => setShowTrainingModal(false)}>
                    <Text style={{ fontSize: 20, color: "#A89F9C" }}>✕</Text>
                  </TouchableOpacity>
                </View>

                <Text style={styles.trainingGuideText}>
                  💡 밝은 곳에서 재료가 잘 보이게 찍힌 사진만 학습에 도움을
                  줍니다!
                </Text>
                ...
                {selectedPhotoForTraining && (
                  <Image
                    source={{
                      uri: `data:image/jpeg;base64,${selectedPhotoForTraining}`,
                    }}
                    style={styles.trainingImagePreview}
                  />
                )}
                ...
                <View style={styles.legalWarningBox}>
                  <Text style={styles.legalWarningText}>
                    🚨 고의로 비(非)식재료를 입력하여 AI 학습을 방해할 시, 심사
                    반려, 앱 이용 제한 및 조치를 당할 수 있습니다.
                  </Text>
                </View>
                ...
                <TouchableOpacity
                  ...
                  onPress={submitTrainingData}
                >
                  <Text style={styles.styleModalBtnTextWhite}>
                    검증 데이터 제출 (심사 후 EXP 지급)
                  </Text>
                </TouchableOpacity>
              </ScrollView>
            </View>
      ...
      >
        <View style={styles.modalOverlayCenter}>
          ...
          >
            ...
            >
              AI에게 알려주시면 더 마음에 드는 메뉴를 추천해 드려요!
            </Text>
            ...
            <View style={styles.styleModalButtons}>
              <TouchableOpacity
                style={styles.styleModalCancel}
                onPress={() => confirmStyleAndGenerate("")}
              >
                <Text style={styles.styleModalBtnText}>건너뛰고 추천받기</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.styleModalSave}
                onPress={() => confirmStyleAndGenerate(preferredStyle)}
              >
                <Text style={styles.styleModalBtnTextWhite}>
                  이 스타일로 요리하기
                </Text>
        ...
        <View style={styles.modalOverlayCenter}>
          <View style={styles.adModalContent}>
            ...
            <TouchableOpacity
              ...
              onPress={() => setAdPromptVisible(false)}
            >
      ...
      <Modal visible={mockAdPlaying} transparent={false} animationType="slide">
        <View style={styles.mockAdContainer}>
          ...
          <Text style={styles.mockAdTimer}>
            {adCountdown}초 후 보상이 지급됩니다
          </Text>
          <ActivityIndicator
            size="large"
            color="#FF8C00"
...
const markdownStyles = StyleSheet.create({
  body: { color: "#F9F5F3", fontSize: 15, lineHeight: 24 },
  heading1: { color: "#FFB347", fontSize: 22, fontWeight: "bold" },
  blockquote: {
...
const styles = StyleSheet.create({
  ...
  bottomMask: {
    backgroundColor: "rgba(42, 36, 33, 0.8)",
    paddingBottom: 40,
    paddingTop: 20,
  ...
  nutritionRow: { gap: 10 },
  nutritionItem: { flexDirection: "row", alignItems: "center", gap: 10 },
  nutritionLabel: { color: "#A89F9C", fontSize: 12, width: 50 },
  nutritionBarBg: {
    flex: 1,
    height: 8,
    backgroundColor: "#5A4E49",
  ...
  disclaimerText: { color: "#8C7A76", fontSize: 11, lineHeight: 16 },
});
