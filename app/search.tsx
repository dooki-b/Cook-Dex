...
export default function SearchPortalScreen() {
  ...
  const [activeTab, setActiveTab] = useState("all"); // 'all' | 'saved' | 'global'
  const [hasSearched, setHasSearched] = useState(false); // 검색 실행 여부 확인
  ...
  // 🚨 검색어 저장 함수
  const saveSearchKeyword = async (term) => {
    if (!term) return;
    let newHistory = [term, ...searchHistory.filter((t) => t !== term)];
    if (newHistory.length > 10) newHistory = newHistory.slice(0, 10); // 최대 10개 유지
  ...
  const handleSearch = async (searchTerm = keyword) => {
    ...
    setKeyword(term);
    saveSearchKeyword(term); // 🚨 검색 시 기록 저장
    Keyboard.dismiss();
    setIsLoading(true);
    setResults([]);
    setHasSearched(true);
      ...
      <View style={styles.header}>
        ...
        <View style={styles.searchBarWrapper}>
          <TextInput
            ...
            onChangeText={(text) => {
              setKeyword(text);
              if (text === "") setHasSearched(false); // 검색어 지우면 초기 화면으로
            }}
