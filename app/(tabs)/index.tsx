...
export default function HomeScreen() {
  ...
  const [nutritionData, setNutritionData] = useState(null); // 🚨 영양 정보 상태 추가
    ...
    useCallback(() => {
      const checkRoulette = async () => {
        try {
          ...
          if (lastSpin === new Date().toLocaleDateString())
    ...
    setTimeout(async () => {
      ...
      if (rewardType.includes("EXP")) {
        ...
        await AsyncStorage.setItem(
          "cookdex_user_exp",
          ((expRaw ? parseInt(expRaw) : 0) + addExp).toString(),
        );
  ...
  const isCurrentPath = (path) => {
    // 탭 내부 경로 처리
    if (path === "/(tabs)" && segments[0] === "(tabs)" && segments.length === 1)
      return true;
    if (path === "/(tabs)/recipes" && segments[1] === "recipes") return true;
    if (path === "/(tabs)/plaza" && segments[1] === "plaza") return true;
    if (path === "/(tabs)/quest" && segments[1] === "quest") return true;
    if (path === "/(tabs)/ranking" && segments[1] === "ranking") return true;
    if (path === "/(tabs)/profile" && segments[1] === "profile") return true;

    // 스택 경로 처리
    if (path === "/search" && segments[0] === "search") return true;
    if (path === "/scanner" && segments[0] === "scanner") return true;
    if (path === "/create-recipe" && segments[0] === "create-recipe")
      return true;
    if (path === "/tutorial" && segments[0] === "tutorial") return true;

    return false;
  };

  const MENU_ITEMS = [
    { icon: "🏠", label: "홈", desc: "메인 대시보드", path: "/(tabs)" },
    { icon: "🔍", label: "검색 포털", desc: "통합 검색", path: "/search" },
    { icon: "📸", label: "AI 스캐너", desc: "재료 인식", path: "/scanner" },
    {
      icon: "📝",
      label: "레시피 생성",
      desc: "맞춤 요리",
    ...
    { icon: "🌍", label: "요리 광장", desc: "커뮤니티", path: "/(tabs)/plaza" },
    { icon: "📜", label: "퀘스트", desc: "미션 & 보상", path: "/(tabs)/quest" },
    { icon: "🏆", label: "랭킹", desc: "명예의 전당", path: "/(tabs)/ranking" },
    {
      icon: "⚙️",
      label: "내 정보",
      desc: "설정 & 프로필",
    ...
    { icon: "📖", label: "튜토리얼", desc: "사용 가이드", path: "/tutorial" },
  ];
        ...
        try {
          ...
          if (limitDataRaw) {
            ...
            if (limitData.date === today) setScansLeft(limitData.left);
              ...
              setScansLeft(DAILY_FREE_LIMIT);
              await AsyncStorage.setItem(
                "cookdex_daily_limit",
                JSON.stringify({ date: today, left: DAILY_FREE_LIMIT }),
              );
            }
          } else
            await AsyncStorage.setItem(
  ...
  const refillQA = async () => {
    try {
      await AsyncStorage.setItem(
        "cookdex_daily_limit",
        JSON.stringify({ date: new Date().toLocaleDateString(), left: 9999 }),
      );
      setScansLeft(9999);
      Alert.alert(
        "🛠️ QA 모드 발동!",
        "AI 카메라 스캔 횟수가 무제한으로 충전되었습니다.",
      );
    } catch (e) {}
  };
  ...
  const handleScanPress = () => {
    if (scansLeft > 0) {
      ...
      Alert.alert(
        "💡 일일 스캔 완료!",
        "오늘의 무료 스캔을 모두 사용하셨습니다. QA 모드를 이용해 충전하세요!",
      );
    }
  };
  ...
  const filteredIngredients = COMMON_INGREDIENTS.filter(
    (i) => i.includes(ingredientSearch) && !selectedIngredients.includes(i),
  );
  const forceScrollToInput = () => {
    setTimeout(() => {
      if (scrollViewRef.current && inputBoxY)
        scrollViewRef.current.scrollTo({
          y: Math.max(0, inputBoxY - windowHeight * 0.25),
          animated: true,
        });
    }, 350);
  };
  ...
  const getCurationThemes = async (stylePreference = "") => {
    ...
    try {
      ...
      let savedDiet = [];
      try {
        savedDiet = savedDietRaw ? JSON.parse(savedDietRaw) : [];
      } catch (e) {
        savedDiet =
          savedDietRaw && savedDietRaw !== "없음" ? [savedDietRaw] : [];
      }
      const savedAllergies = await AsyncStorage.getItem("cookdex_allergies");
      const savedCondimentsRaw =
        await AsyncStorage.getItem("cookdex_condiments");

      const dietText =
        savedDiet.length > 0
          ? `[식단 목표: ${savedDiet.join(", ")}]에 맞춰서 요리해 줘.`
          : "";
      const allergyText =
        savedAllergies && savedAllergies !== "없음"
          ? `[🚨치명적 경고🚨 알레르기 및 기피 재료: ${savedAllergies}] 이 재료들은 레시피에 절대 포함시키지 마!`
          : "";
      const condimentsText = savedCondimentsRaw
        ? `[보유 중인 기본 양념/향신료: ${JSON.parse(savedCondimentsRaw).join(", ")}] 이 재료들은 이미 집에 있으니 '부족한 구매 리스트'에서 빼고 자유롭게 써 줘.`
        : "";
  ...
  const generateFinalRecipe = async (theme) => {
    ...
    try {
      ...
      const systemPrompt = `너는 셰프야. 재료(${selectedIngredients.join(", ")})를 가지고 [${theme.theme_title}] 레시피를 작성해.${userContextPrompt}\n⚠️ 필수 지시사항: 식약처 등 공신력 있는 영양 데이터베이스를 참고하여 1인분 기준 총 칼로리(kcal)와 3대 영양소(탄수화물, 단백질, 지방) 함량을 추정해.\n반드시 아래 JSON 형식으로만 대답해. 마크다운(\`\`\`json 등) 절대 금지.\n{ "safety_warning": "위생 경고 필요시 작성, 없으면 null", "substitutions": [ { "original": "필요한데 유저가 입력 안한 재료", "substitute": "대체재", "reason": "대체 이유" } ], "shopping_list": ["대체불가 필수 마트 구매 재료"], "nutrition_info": { "calories": 0, "carbs": 0, "protein": 0, "fat": 0 }, "recipe_markdown": "무조건 첫 줄은 '# ${theme.theme_title}'. 영양 정보는 텍스트로 적지 마. 조리 순서는 무조건 '1. ', '2. ' 같은 숫자로 시작할 것." }`;
      ...
      if (parsedData.safety_warning && parsedData.safety_warning !== "null")
        Alert.alert("🚨 쿡덱스 위생/안전 경고!", parsedData.safety_warning);
      setShoppingList(
        parsedData.shopping_list && Array.isArray(parsedData.shopping_list)
          ? parsedData.shopping_list
          : [],
      );
      if (parsedData.nutrition_info)
        setNutritionData(parsedData.nutrition_info);
      ...
      if (parsedData.substitutions && parsedData.substitutions.length > 0) {
        ...
        let subText = "\n> **💡 쿡덱스 스마트 대체재 추천**\n";
        parsedData.substitutions.forEach((s) => {
          subText += `> - ❌ ${s.original} ➡️ ⭕ **${s.substitute}** (${s.reason})\n`;
        });
        subText += "\n";
        if (titleMatch)
          finalMarkdown = finalMarkdown.replace(
            titleMatch[0],
            titleMatch[0] + subText,
          );
      ...
      await AsyncStorage.setItem(
        "cookdex_draft_recipe",
        JSON.stringify({
          ingredients: selectedIngredients,
          recipe: finalMarkdown,
          shopping: parsedData.shopping_list,
          nutrition: parsedData.nutrition_info,
        }),
      );
    } catch (error) {
      Alert.alert("에러", "레시피 생성 실패");
      setShowBottomModal(false);
    } finally {
  ...
  const startCookingMode = () => {
    ...
    const extractedSteps = textRecipeResult
      .split("\n")
    ...
    if (extractedSteps.length === 0) {
      Alert.alert(
        "알림",
        "조리 단계를 명확히 인식하지 못했습니다. 일반 텍스트 모드를 이용해주세요.",
      );
      return;
    }
  ...
  const handleRecipeSaveAndShare = async (isSharing) => {
    ...
    try {
      ...
      const currentExp = parseInt(
        (await AsyncStorage.getItem("cookdex_user_exp")) || "0",
      );
      ...
      if (currentUser) {
        ...
        if (isSharing) {
          ...
          Alert.alert(
            "광장에 등록 완료! 🌍✨",
            `레시피를 공유하여 엄청난 보상을 받았습니다! (+${earnedExp} EXP)`,
          );
        } else
          Alert.alert(
            "내 주방 저장 완료! 🍳",
            `레시피가 조용히 저장되었습니다. (+${earnedExp} EXP)`,
          );
      }
    } catch (error) {
      alert("저장 에러");
  ...
  const requestNewStyle = () => {
    let finalType =
      isCustomType && customTypeInput.trim() !== ""
        ? customTypeInput
        : selectedType;
    let finalTaste =
      isCustomTaste && customTasteInput.trim() !== ""
        ? customTasteInput
        : selectedTaste;
    let styleStr = "";
    ...
    if (!styleStr) {
      Alert.alert("알림", "원하시는 요리 종류나 맛을 선택해주세요!");
      return;
    }
    setShowStyleModal(false);
    getCurationThemes(styleStr);
  };

  const currentLevelInfo = calculateLevel(userExp);
  const expProgress =
    currentLevelInfo.level === "MAX"
      ? 100
      : (userExp / currentLevelInfo.nextExp) * 100;
  ...
  return (
    <SafeAreaView style={styles.container}>
          ...
          <View style={styles.legalModalContent}>
            ...
            >
              <Text style={styles.legalMainText}>
                AI 셰프 쿡덱스를 이용하기 전, 사용자의 안전을 위해 반드시 아래
                내용을 확인해 주세요.
              </Text>

              <View style={styles.legalPointBox}>
                <Text style={styles.legalPointTitle}>
                  🥩 1. 조리 및 위생 주의
                </Text>
                <Text style={styles.legalPointDesc}>
                  AI가 제안하는 조리 시간과 온도는 대략적인 참고용입니다. 고기,
                  해산물 등은 반드시 본인의 판단하에 속까지 안전하게 완전히 익혀
                  드셔야 합니다.
                </Text>
              </View>
              ...
              <View style={styles.legalPointBox}>
                ...
                <Text style={styles.legalPointDesc}>
                  생성된 레시피에 본인에게 치명적인 알레르기 유발 물질이
                  포함되어 있는지, 조리 전에 유저 스스로 직접 교차 검증해야
                  합니다.
                </Text>
              </View>
              ...
              <View style={styles.legalPointBox}>
                ...
                <Text style={styles.legalPointDesc}>
                  AI는 시각적으로 완벽하지 않으며, 가끔 먹을 수 없는 물건(예:
                  모형, 화학제품 등)을 식재료로 착각하여 요리를 제안할 수
                  있습니다. 비식재료는 절대 취식하지 마십시오.
                </Text>
              </View>

              <Text style={styles.legalFooterText}>
                ※ 위 사항을 무시하여 발생한 취식 상의 문제 및 신체적 피해에
                대하여 앱 개발자는 어떠한 법적 책임도 지지 않음을 명시합니다.
              </Text>
            </ScrollView>

            <TouchableOpacity
              style={styles.legalAgreeButton}
              onPress={handleAgreeLegal}
            >
              <Text style={styles.legalAgreeButtonText}>
                위 모든 내용을 숙지하였으며, 동의합니다
              </Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      >
        ...
        >
          <View style={styles.header}>
            ...
            {/* 🚨 검색 포털 진입 버튼 추가 */}
            <TouchableOpacity
              style={styles.portalSearchBtn}
              onPress={() => router.push("/search")}
              activeOpacity={0.9}
            >
              <Text style={styles.portalSearchText}>
                🔍 레시피, 재료, 셰프 검색...
              </Text>
            </TouchableOpacity>
            <View>
          ...
          <View style={{ flexDirection: "row", gap: 10, marginBottom: 20 }}>
            ...
            <TouchableOpacity
              style={[styles.qaButton, { flex: 1, backgroundColor: "#5A4E49" }]}
              onPress={resetLegal}
            >
          ...
          >
            <ImageBackground
              source={{
                uri: "https://images.unsplash.com/photo-1495195129352-aeb325a55b65?q=80&w=2076&auto=format&fit=crop",
              }}
          ...
          <View style={styles.categorySection}>
            ...
            >
              <TouchableOpacity style={styles.categoryCard}>
                ...
                <Text style={styles.categoryText}>바쁜 아침 10분컷</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.categoryCard}>
                <Text style={styles.categoryEmoji}>🍻</Text>
                <Text style={styles.categoryText}>퇴근 후 맥주 안주</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.categoryCard}>
                <Text style={styles.categoryEmoji}>🥗</Text>
                <Text style={styles.categoryText}>다이어트 식단</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.categoryCard}>
                <Text style={styles.categoryEmoji}>🤧</Text>
                <Text style={styles.categoryText}>감기 기운 있을 때</Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
        </ScrollView>
      ...
      <Modal
        ...
        transparent={true}
        animationType="slide"
        onRequestClose={() => !isSpinning && setRouletteVisible(false)}
      >
        <View style={styles.modalOverlay}>
          <TouchableOpacity
            style={StyleSheet.absoluteFill}
            activeOpacity={1}
            onPress={() => !isSpinning && setRouletteVisible(false)}
          />
          ...
          <View style={styles.bottomSheetContainer}>
            ...
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitleText}>
                {textRecipeResult ? "✨ AI 텍스트 레시피" : "🎯 맞춤 요리 제안"}
              </Text>
              <TouchableOpacity
                  ...
                  <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    {curationThemes.map((theme, index) => (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.themeCard,
                          { borderColor: theme.ui_accent_color },
                        ]}
                        onPress={() => generateFinalRecipe(theme)}
                      >
                        <View
                          style={{
                            flexDirection: "row",
                        ...
                        <View
                          style={{
                            alignSelf: "flex-end",
                            marginTop: 10,
                            backgroundColor: theme.ui_accent_color,
                    ...
                    <View style={styles.nutritionCard}>
                          ...
                          <View style={styles.nutritionBarBg}>
                            <View
                              style={[
                                ...
                                {
                                  width: `${Math.min(nutritionData.carbs / 3, 100)}%`,
                          ...
                          <View style={styles.nutritionBarBg}>
                            <View
                              style={[
                                styles.nutritionBarFill,
                                {
                                  width: `${Math.min(nutritionData.protein / 1, 100)}%`,
                          ...
                          <View style={styles.nutritionBarBg}>
                            <View
                              style={[
                                styles.nutritionBarFill,
                                {
                                  width: `${Math.min(nutritionData.fat / 1, 100)}%`,
                      ...
                      <View style={styles.disclaimerBox}>
                        <Text style={styles.disclaimerText}>
                          ⚠️ AI가 식약처 DB를 참고하여 추정한 값이며, 실제
                          조리법과 식재료 상태에 따라 오차가 발생할 수 있습니다.
                        </Text>
                      </View>
                    </View>
                  )}
                      ...
                      >
                        {shoppingList.map((item, idx) => (
                          <TouchableOpacity
                            ...
                            onPress={() => handleShopping(item)}
                          >
                            <Text style={styles.commerceBtnText}>
                              {item} 검색 🔍
                            </Text>
                          </TouchableOpacity>
                        ))}
            ...
            {!isGeneratingRecipe && textRecipeResult && (
              <View style={styles.resultButtonsGrid}>
                <TouchableOpacity
                  style={[
                    styles.gridBtn,
                    { backgroundColor: "#FF6B6B", flex: 0.8 },
                  ]}
                  onPress={() => handleRecipeSaveAndShare(false)}
                >
                  <Text style={styles.gridBtnText}>저장</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    styles.gridBtn,
                    { backgroundColor: "#4CAF50", flex: 1.1 },
                  ]}
                  onPress={() => handleRecipeSaveAndShare(true)}
                >
                  <Text style={styles.gridBtnText}>광장 공유 🌍</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    styles.gridBtn,
                    { backgroundColor: "#8E24AA", flex: 1.4 },
                  ]}
                  onPress={() => {
                    setShowStyleModal(true);
                  }}
                >
                  <Text style={styles.gridBtnText}>레시피 수정 🎲</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
            ...
            <ScrollView>
              ...
              <View style={styles.styleTagsContainer}>
                ...
                <TouchableOpacity
                  style={[
                    styles.styleTag,
                    isCustomType && styles.styleTagActive,
                  ]}
                  onPress={() => {
                ...
                >
                  <Text
                    style={[
                      styles.styleTagText,
                      isCustomType && styles.styleTagTextActive,
                    ]}
                  >
                    + 직접 입력
                  </Text>
              ...
              <Text style={[styles.styleInputLabel, { marginTop: 20 }]}>
                2. 맛 / 분위기 선택 (선택사항)
              </Text>
              <View style={styles.styleTagsContainer}>
                {RECIPE_TASTES.map((taste) => (
                  <TouchableOpacity
                    key={taste}
                    style={[
                      styles.styleTag,
                    ...
                    onPress={() => {
                      setIsCustomTaste(false);
                      setSelectedTaste(taste === selectedTaste ? "" : taste);
                    }}
                  >
                    <Text
                      style={[
                ...
                <TouchableOpacity
                  style={[
                    styles.styleTag,
                    isCustomTaste && styles.styleTagActive,
                  ]}
                  onPress={() => {
                    setIsCustomTaste(true);
                    setSelectedTaste("");
                  }}
                >
                  <Text
                    style={[
                      styles.styleTagText,
                      isCustomTaste && styles.styleTagTextActive,
                    ]}
                  >
                    + 직접 입력
                  </Text>
              ...
              <View style={styles.styleModalButtons}>
                ...
                >
                  <Text style={styles.styleModalBtnText}>취소</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.styleModalSave}
                  onPress={requestNewStyle}
                >
                  <Text style={styles.styleModalBtnTextWhite}>추천받기 ✨</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
        ...
        >
          <View style={styles.menuContent}>
            <View style={styles.menuHeader}>
              <Text style={styles.menuTitle}>전체 메뉴 🗺️</Text>
            ...
            <View style={styles.menuGrid}>
              {MENU_ITEMS.map((item, index) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.menuItem,
                    isCurrentPath(item.path) && styles.menuItemActive,
                  ]}
                  onPress={() => navigateFromMenu(item.path)}
                >
                  ...
                  <View style={{ flex: 1 }}>
                    <Text
                      style={[
                        styles.menuItemText,
                        isCurrentPath(item.path) && styles.menuItemTextActive,
                      ]}
                    >
                      {item.label}
                    </Text>
                    <Text
                      style={[
                        styles.menuItemDesc,
                        isCurrentPath(item.path) && styles.menuItemDescActive,
                      ]}
                    >
                      {item.desc}
                    </Text>
...
const styles = StyleSheet.create({
  ...
  logoText: { fontSize: 24, fontWeight: "900", color: "#FF8C00" },
  menuBtn: { padding: 5 },
  menuBtnText: { fontSize: 28, color: Colors.textMain, fontWeight: "bold" },
  portalSearchBtn: {
    width: "100%",
  ...
  portalSearchText: { color: "#A89F9C", fontSize: 15, fontWeight: "600" },
  greeting: {
    fontSize: 22,
    fontWeight: "900",
  ...
  manualSubmitBtn: {
    ...
    borderRadius: 14,
    marginTop: 10,
  }, // Keep specific color for success action
